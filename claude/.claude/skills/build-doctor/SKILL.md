...
- Prefer any "Loaded from" base path the host exposes for this skill (Claude Code's Skill tool result includes it). Use that as the authoritative loaded path.
...
```bash
readlink ~/.claude/skills/build-doctor
# or, for full visibility:
ls -l ~/.claude/skills/build-doctor
```

The Claude symlink target points into `<bundle>/.claude/skills/build-doctor`. Walk up three levels to the bundle root and read its `VERSION`.
