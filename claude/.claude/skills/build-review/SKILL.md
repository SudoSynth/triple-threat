...
Invoke the `review` skill via the Skill tool (bare name, no leading slash; this is GStack's `review`, not Claude Code's built-in `/review`). This is GStack's staff-engineer review — finds bugs, smells, anti-patterns, and auto-fixes obvious issues with atomic commits.
