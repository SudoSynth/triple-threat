...
1. **Triple Threat orchestrator installed?** Run `ls ~/.claude/skills/build-feature/SKILL.md`. If missing, the user needs to install Triple Threat (see `~/.claude/skills/triple-threat/`).

2. **GSD installed?** Run `ls ~/.claude/skills/gsd-new-project/ ~/.claude/skills/gsd-execute-phase/`. If missing: `npx get-shit-done-cc@latest --claude --global`.

3. **GStack installed?** Run `ls ~/.claude/skills/gstack/autoplan/SKILL.md`. If missing: clone `https://github.com/garrytan/gstack.git` to `~/.claude/skills/gstack/` and run its `./setup --host claude` (requires `bun` runtime; install via `brew install oven-sh/bun/bun`).

4. **Superpowers installed?** Run `ls ~/.claude/skills/superpowers-test-driven-development/SKILL.md`. If missing: clone `https://github.com/obra/superpowers.git` to `~/.claude/plugins/superpowers/` and symlink each skill dir into `~/.claude/skills/` with a `superpowers-` prefix.
...
For most work, just type `/build-feature <what you want>`. The other commands are escape hatches for when reality breaks the happy path.
...
CLAUDE.md (this file)             ← project onboarding + behavioral guardrails
```
...
This project is set up for multi-session work. If a feature takes more than one Claude session:
...
- `~/.claude/skills/triple-threat/` — orchestrator skill source if you need to inspect or modify
- `~/.claude/skills/gsd-help` — full GSD command catalog
- `~/.claude/skills/gstack/README.md` — full GStack documentation
