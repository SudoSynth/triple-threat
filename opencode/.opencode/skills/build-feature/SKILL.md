...
Local Git history is required for the pipeline. GitHub is optional. Run these checks first, before any `.planning/` work, before AGENTS.md deployment, before codebase mapping. The pipeline depends on durable Git history at every later stage; failing fast here prevents writing planning artifacts into a folder with no Git backing.
...
4. **If the output shows other untracked files** (folder was non-empty / inherited):
   ...
   - Options:
     ...
     - **"Cancel"** — exit cleanly. Do NOT proceed to create `.planning/`, `AGENTS.md`, or any other artifacts. The Git repo and `.gitignore` already exist from Checks 0.2-0.3 and remain in place uncommitted.
...
### Check 2: Deploy AGENTS.md template
```
Does AGENTS.md exist in the current working directory?
  YES → leave it alone (don't overwrite the user's existing AGENTS.md). Continue.
  NO  → copy the Triple Threat onboarding template into the project root, then
        commit it explicitly via Bash. Prefer the project-local template when
        present; otherwise fall back to the global install:
          if [ -f .opencode/skills/triple-threat/AGENTS.md.template ]; then
            cp .opencode/skills/triple-threat/AGENTS.md.template ./AGENTS.md
          else
            cp ~/.config/opencode/skills/triple-threat/AGENTS.md.template ./AGENTS.md
          fi
          git add AGENTS.md
          git commit -m "docs: deploy Triple Threat onboarding (AGENTS.md)"
        Commit explicitly here — do NOT rely on a downstream `gsd-sdk query commit`
        call to capture AGENTS.md, because gsd-sdk has been observed to silently
        drop paths from its `files:` array, leaving AGENTS.md untracked through
        the rest of the pipeline. Committing in pre-flight makes it durable
        immediately. Future OpenCode sessions in this directory will auto-load it.
```

### Check 3: Codebase map
```
If --remap flag was passed:
  Force-run the `build-map` skill regardless of cache state.
...
Else:
  Does .planning/codebase/ exist AND contain STACK.md AND ARCHITECTURE.md?
    ...
    NO  → invoke the `build-map` skill via the Skill tool (uses gsd-map-codebase under the hood)
...
Invoke the `build-spec` skill via the Skill tool, passing the user's feature description.
...
Invoke the `build-plan` skill via the Skill tool. This will:
...
Invoke the `build-exec` skill via the Skill tool. This wraps `gsd-execute-phase` but injects the Superpowers TDD discipline into each per-task subagent.

The exec stage handles its own approval gates per wave if needed. Wait for all waves to complete.

## Stage 4: REVIEW

Invoke the `build-review` skill via the Skill tool. This will:
...
Otherwise, invoke the `build-qa` skill via the Skill tool. The QA skill will detect whether the project has UI by inspecting `.planning/codebase/STACK.md` and `package.json`. If no UI is detected, it skips silently with a notice. If UI is detected, it runs GStack's `/qa` (real-browser test-fix-verify loop).

## Stage 6: SHIP

Invoke the `build-ship` skill via the Skill tool. This wraps GStack's `/ship`:
