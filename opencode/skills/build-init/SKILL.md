...
Sets up everything `/build-feature` needs in a repo, but stops short of starting any feature work. Use this when you've inherited a codebase and want it prepped for future OpenCode work, or when you want to read the codebase analysis before deciding what to build.
...
4. **If other untracked files show up** (non-empty / inherited folder):
   ...
   - Options:
     ...
     - **"Cancel"** — exit cleanly. Do NOT proceed to create `.planning/`, `AGENTS.md`, or run codebase mapping. The Git repo and `.gitignore` remain in place.
...
### 2. Deploy the AGENTS.md template

Check if `AGENTS.md` exists in the current working directory.
- If yes → leave it alone (don't overwrite the user's existing AGENTS.md). Skip to step 3.
- If no → copy the Triple Threat AGENTS.md template into the project root and commit it explicitly using the Bash tool. Prefer the project-local template when present; otherwise use the global install fallback:
  ```bash
  ...
  ```
  Commit explicitly here — do NOT rely on a downstream `gsd-sdk query commit` to capture AGENTS.md, because the SDK has been observed to silently drop paths from its `files:` array, leaving AGENTS.md floating untracked. Committing in this step makes the onboarding doc durable in history immediately. The template includes the setup checklist, command quick reference, and operating principles.
...
✓ AGENTS.md deployed (or kept existing)
